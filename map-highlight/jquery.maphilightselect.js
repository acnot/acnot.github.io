...
(function($) {
	...
	if(!(has_canvas || has_VML)) {
		$.fn.maphilightselect = function() { return this; };
		return;
	}
	...
	options_from_area = function(area, options) {
		var $area = $(area);
		return $.extend({}, options, $.metadata ? $area.metadata() : false, $area.data('maphilightselect'));
	};
	...
	$.fn.maphilightselect = function(opts) {
		opts = $.extend({}, $.fn.maphilightselect.defaults, opts);
		...
		return this.each(function() {
			var img, wrap, options, map, canvas, canvas_always, mouseover, click, highlighted_shape, usemap, lists = {};
			img = $(this);

			if(!is_image_loaded(this)) {
				// If the image isn't fully loaded, this won't work correctly.  Need to try again later.
				return window.setTimeout(function() {
					img.maphilightselect(opts);
				}, 200);
			}

			options = $.extend({}, opts, $.metadata ? img.metadata() : false, img.data('maphilightselect'));
			...
			if(img.hasClass('maphilightselected')) {
				// We're redrawing an old map, probably to pick up changes to the options.
				// Just clear out all the old stuff.
				var wrapper = img.parent();
				img.insertBefore(wrapper);
				wrapper.remove();
				$(map).unbind('.maphilightselect').find('area[coords]').unbind('.maphilightselect');
			}
			...
			click = function(e) {
				e.preventDefault();
				if (!this[options.id_prop]) return false;
				...
				if (1 === mode && lists && lists[mode]) {
					var i = 0;
					$.each(lists[mode], function(){ i++ });
					if (i >= options.max_selected) return true;
				}
				...
				data.alwaysOn = select;
				$this.data('maphilightselect', data).trigger('alwaysOn.maphilightselect');
				$this.data('mode', mode);
			...
			$(map).bind('alwaysOn.maphilightselect', function() {
			...
			$(map).trigger('alwaysOn.maphilightselect').find('area[coords]')
				.bind('mouseover.maphilightselect', mouseover)
				.bind('click.maphilightselect', click)
				.bind('mouseout.maphilightselect', function(e) { clear_canvas(canvas); });
			
			img.before(canvas); // if we put this after, the mouseover events wouldn't fire.
			
			img.addClass('maphilightselected');

			var preselect = function($el, mode) {
				$.each($el.val().split(options["separator"]), function(i, value){
					if (value === "") return;
